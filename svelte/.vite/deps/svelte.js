      ...
      if (!skip && effect2.fn !== null) {
        if (is_branch) {
          ...
          if ((effect2.f & BLOCK_EFFECT) !== 0)
            this.#dirty_effects.add(effect2);
...
function internal_set(source2, value) {
  if (!source2.equals(value)) {
    ...
    if ((source2.f & DERIVED) !== 0) {
      ...
      set_signal_status(
        source2,
        (source2.f & CONNECTED) !== 0 ? CLEAN : MAYBE_DIRTY,
      );
    }
...
function get(signal) {
  ...
  if (is_destroying_effect) {
    ...
    (!batch_values?.has(signal) ||
      (current_batch?.is_fork && !effect_tracking()))
  ) {
